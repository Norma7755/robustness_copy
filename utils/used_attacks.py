...
class Lyap_Control(Attack):
    ...
    def forward(self, images):
        ...
        for _ in range(self.steps):
            ...
            outputs, lyap_loss = self.model(adv_images, ret_lyap = True)
...
class PGD(Attack):
    ...
    """
    def __init__(self, model, eps=16/255,
                 alpha=2/255, steps=40, random_start=True,loss=nn.CrossEntropyLoss(),targeted=False, target=None, normalize = True):
        super().__init__("PGD", model)
        self.eps = eps
        self.alpha = alpha
        self.steps = steps
...
class Grad_MIFGSM(Attack):
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            if self.targeted:
                cost = -loss(outputs, labels)
                grad = torch.autograd.grad(cost, normalized_adv_images,
                                       retain_graph=True, create_graph=True)[0]
                
                cost = torch.norm(grad,p=2)*cost.sum()
            else:
                cost = loss(outputs, labels)
...
class PGD_DrawBack(Attack):
    ...
    Shape:
        - input/images: :math:`(N, H, L)` where `N = number of batches`, `H = number of Hidden States`, `L = Input Length`. It must have a range [0, 1].
        - output: :math:`(N, H, L)` as the same shape as input.


    """
    def __init__(self, model, eps=4/255, alpha=1/255, steps=10, random_start=False):
        super().__init__("PGD_DrawBack", model)
        self.eps = eps
        self.alpha = alpha
        self.steps = steps
        self.random_start = random_start
    ...
    def forward(self, images):
        ...
        adv_delta = torch.zeros_like(images).to(self.device)
        # self.eps = float(torch.norm(images, p = float('inf')) * 0.03)
        # self.alpha = self.eps/10
        clean_images = images.clone().detach()

        if self.random_start:
            # Starting at a uniformly random point
            adv_delta = adv_delta + torch.empty_like(adv_delta).uniform_(-self.eps, self.eps) * 10
            adv_delta = torch.clamp(adv_delta, min=-self.eps, max=self.eps)
            clean_images = torch.clamp(clean_images+adv_delta, min=0, max=1).detach()
        ...
        for _ in range(self.steps):
            ...
            # print(adv_images)
            # out, _, _, kl_loss = self.model(adv_images, detach = False)
            out, kl_loss = self.model(adv_images)
            # kl_loss = self.model.get_u_KLloss()
            # print(kl_loss)
            # Calculate loss
            cost = -kl_loss
            # print(cost)
            ...
            adv_delta = torch.clamp(adv_delta, min=-self.eps, max=self.eps)
        adv_images = torch.clamp(images + adv_delta, min=0, max=1).detach()
        # adv_images = images + adv_delta

        return adv_images    

class Grad_TIFGSM(Attack):
    r"""
    TIFGSM in the paper 'Evading Defenses to Transferable Adversarial Examples by Translation-Invariant Attacks'
    [https://arxiv.org/abs/1904.02884]
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            adv_images = torch.clamp(clean_images+adv_delta, min=0, max=1)
            if self.normalize:
                normalized_adv_images = self.norm(self.input_diversity(adv_images))
                outputs = self.model(normalized_adv_images)
            else:
                outputs = self.model(self.input_diversity(adv_images))

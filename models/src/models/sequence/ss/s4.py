...
from models.src.models.sequence.ss.kernel import HippoSSKernel
...
class S4(nn.Module):
    ...
    def __init__(
        ...
        bidirectional=False,
        # Arguments for FF
        activation="glu",  # activation in between SS and FF
        ...
        **kernel_args,
        ...
        self.kernel = HippoSSKernel(
            self.h, N=self.n, L=l_max, channels=channels, verbose=verbose, trainable=True, **kernel_args
        )
        ...
        if not self.linear:
            # if activation == 'glu':
            #     self.glu = True
            #     self.reshape_linear = nn.Linear(d_model, 2 * d_model)
            # else:
            #     self.glu = False
